    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Book` (`id` TEXT, `title` TEXT, `status` TEXT, `student` TEXT NOT NULL, `pages` INTEGER, `usedCount` INTEGER, PRIMARY KEY (`id`))');
  ...
  Future<List<Book>> findAllBooks() async {
    return _queryAdapter.queryList('SELECT * FROM Book',
        mapper: (Map<String, Object?> row) => Book(
            ...
            usedCount: row['usedCount'] as int?));
  }
